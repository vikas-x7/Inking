  ...
  return (
    <header className="relative z-30 flex h-9 shrink-0 select-none items-center justify-between border-b border-white/5 bg-[#252526] text-white px-2">
      <div className="flex items-center gap-x-1 overflow-x-auto scrollbar-none">
        ...
        <button
          onClick={() => setPickerView('archived')}
          className={`${actionButtonClass} hidden sm:flex`}
        >
          Archive
        </button>
        ...
        <div className="relative" ref={layoutMenuRef}>
          ...
          {showLayoutMenu && (
            <div
              ...
              className="fixed z-50 w-60 select-none rounded-[5px]  bg-[#252526] p-2 shadow-2xl text-xs"
            >
              <div className="mb-1 px-2.5 py-1 text-[12px] font-normal text-white/60">
                Layout options
              </div>
              ...
              <div className="flex flex-col gap-0.5">
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-[3px] px-2.5 py-2 text-sm font-normal text-left transition cursor-pointer ${
                    layoutMode === 'split'
                      ? 'bg-[#0052EA] font-medium text-white'
        ...
        <button
          ...
          disabled={!pdfUrl}
          className={`${actionButtonClass} disabled:cursor-not-allowed disabled:opacity-30`}
        >
          Download
        </button>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-2 pl-2 sm:gap-3">
        {saveState !== 'idle' && (
          <span
            className={`text-[11px] font-medium whitespace-nowrap ${saveState === 'error' ? 'text-red-400' : 'text-white/50'}`}
          >
