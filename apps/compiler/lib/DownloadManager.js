...
var TarExtractor = require('./TarExtractor');
    ...
    createTarballExtractor(tarballPath) {
        ...
        return new Downloader(async () => {
            ...
            if (!success)
                return new DownloadResult(null, null);
            try {
                logger.info(`extracting tarball ${tarballPath} into ${folderPath}`);
                await TarExtractor.extractArchive(tarballPath, folderPath);
            } catch (e) {
                utils.rmdirRecursive(folderPath);
                logger.error(`ERROR: Downloader failed to extract tarball. ${e.message}`);
                return new DownloadResult(null, e.message);
            }
            return new DownloadResult(folderPath, null);
        });
    }
